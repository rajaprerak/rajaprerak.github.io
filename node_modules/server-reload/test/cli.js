...
var cmd = path.join(__dirname, "..", "server-reload.js");
var opts = {
	timeout: 2000,
	maxBuffer: 1024
};
...
describe('command line usage', function() {
	it('--version', function(done) {
		exec_test([ "--version" ], function(error, stdout, stdin) {
			assert(!error, error);
			assert(stdout.indexOf("server-reload") === 0, "version not found");
			done();
		});
	});
	it('--help', function(done) {
		exec_test([ "--help" ], function(error, stdout, stdin) {
			assert(!error, error);
			assert(stdout.indexOf("Usage: server-reload") === 0, "usage not found");
			done();
		});
	});
