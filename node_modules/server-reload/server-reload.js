...
var ServerReload = require("./index");
...
var configPath = path.join(homeDir, '.server-reload.json');
...
for (var i = process.argv.length - 1; i >= 2; --i) {
	...
	if (arg.indexOf("--port=") > -1) {
		...
		console.log('Usage: server-reload [-v|--version] [-h|--help] [-q|--quiet] [--port=PORT] [--host=HOST] [--open=PATH] [--no-browser] [--browser=BROWSER] [--ignore=PATH] [--ignorePattern=RGXP] [--no-css-inject] [--entry-file=PATH] [--spa] [--mount=ROUTE:PATH] [--wait=MILLISECONDS] [--htpasswd=PATH] [--cors] [--https=PATH] [--https-module=MODULE_NAME] [--proxy=PATH] [PATH]');
		process.exit();
	}
	else if (arg === "--test") {
		// Hidden param for tests to exit automatically
		setTimeout(ServerReload.shutdown, 500);
		process.argv.splice(i, 1);
	}
}
