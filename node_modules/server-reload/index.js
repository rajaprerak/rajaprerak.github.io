  ...
  return function (req, res, next) {
    ...
    function file(filepath /*, stat*/) {
      ...
      if (hasNoOrigin && possibleExtensions.indexOf(x) > -1) {
        ...
        if (injectTag === null && ServerReload.logLevel >= 3) {
          console.warn(
 ...
 */
ServerReload.start = function (options) {
  options = options || {}
  ...
  ServerReload.logLevel = options.logLevel === undefined ? 2 : options.logLevel
  var openPath =
  ...
  } else if (ServerReload.logLevel > 2) {
    app.use(logger('dev'))
  }
  if (options.spa) {
    middleware.push('spa')
  ...
  server.addListener('error', function (e) {
    if (e.code === 'EADDRINUSE') {
      ...
      ServerReload.shutdown()
    }
  })

  // Handle successful server
  server.addListener('listening', function (/*e*/) {
    ServerReload.server = server
    ...
    if (ServerReload.logLevel > 2 && address.address === '0.0.0.0') {
  ...
  // Setup file watcher
  ServerReload.watcher = chokidar.watch(watchPaths, {
    ignored: ignored,
    ignoreInitial: true,
  })
  ...
  ServerReload.watcher
    ...
    .on('ready', function () {
      if (ServerReload.logLevel >= 1) console.log('Ready for changes'.cyan)
    })
...
ServerReload.shutdown = function () {
  var watcher = ServerReload.watcher
  if (watcher) {
    watcher.close()
  }
  var server = ServerReload.server
